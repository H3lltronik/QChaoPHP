<?php
    ...
    if ($conn) {
        $qContenido = mysqli_query($conn, "SELECT * FROM ( (SELECT * FROM getshitpostdata ORDER BY idShitpost DESC LIMIT 5) AS SUB ) ORDER BY idShitpost ASC;") or die(mysqli_error($conn));
        if ($qContenido) {
            $contContenido = 0;
